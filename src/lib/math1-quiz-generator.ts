import { CATEGORIES, MATH1_QUESTIONS } from "@/lib/math1QuestionBank";
...
function poolFor(category: string): Question[] {
  return MATH1_QUESTIONS.filter((q) => q.category === category);
}

// 1問だけランダムに出す(連続回答モード用)
export function generateQuestion(): Question {
  return MATH1_QUESTIONS[Math.floor(Math.random() * MATH1_QUESTIONS.length)];
}
