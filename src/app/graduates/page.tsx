...
import { redis, GRADUATES_KEY, parseZRangeWithScores } from "@/lib/redis";

export const revalidate = 30;

export default async function GraduatesPage() {
  const flat = await redis.zrange<string[]>(GRADUATES_KEY, 0, -1, { rev: true, withScores: true });
    ...
    <div className="flex min-h-dvh w-full flex-col items-center bg-gradient-to-b from-sky-100 via-violet-100 to-rose-100 px-4 pb-8 pt-[max(2.5rem,env(safe-area-inset-top))]">
      <div className="flex w-full max-w-md flex-1 flex-col gap-6">
        <div className="text-center">
          ...
          <p className="mt-3 text-slate-500">全カテゴリクリア + 卒業テスト合格者</p>
        </div>
        ...
        <Link
          href="/"
          className="mt-auto w-full rounded-2xl bg-white py-4 text-center text-lg font-semibold text-violet-500 shadow-md transition-transform active:scale-95"
        >
          トップへもどる
        </Link>
      </div>
