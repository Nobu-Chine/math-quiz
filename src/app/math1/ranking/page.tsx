...
import { redis, MATH1_STREAK_BEST_KEY, parseZRangeWithScores } from "@/lib/redis";
...
export default async function Math1RankingPage() {
  const flat = await redis.zrange<string[]>(MATH1_STREAK_BEST_KEY, 0, 4, {
    rev: true,
    withScores: true,
  });
    ...
    <div className="flex min-h-dvh w-full flex-col items-center bg-gradient-to-b from-amber-100 via-orange-100 to-rose-100 px-4 pb-8 pt-[max(2.5rem,env(safe-area-inset-top))]">
      <div className="flex w-full max-w-md flex-1 flex-col gap-6">
        <div className="text-center">
          <p className="text-sm font-semibold tracking-wide text-amber-600">中学1年 数学</p>
        ...
        <Link
          href="/math1"
          className="mt-auto w-full rounded-2xl bg-white py-4 text-center text-lg font-semibold text-amber-600 shadow-md transition-transform active:scale-95"
        >
          数学クイズにもどる
        </Link>
      </div>
